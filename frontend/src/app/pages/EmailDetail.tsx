      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
            Màn này dùng để mở sâu vào email gốc khi người dùng muốn kiểm tra nội dung email, tệp
            PDF và các thông tin đã được trích xuất.
          </p>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-6">
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 sm:p-8">
              ...
              <div className="mt-5 space-y-4 text-sm">
                ...
                <div>
                  <p className="text-slate-500">Tiêu đề email</p>
                  <p className="mt-1 font-medium text-slate-900">{detail.email.subject}</p>
                </div>
                <div>
                  <p className="text-slate-500">Thời điểm gửi</p>
                ...
                <div>
                  <p className="text-slate-500">Tóm tắt ngắn</p>
                  <p className="mt-1 text-slate-900">{detail.email.snippet || '-'}</p>
                </div>
                <div>
                  <p className="text-slate-500">Nội dung email</p>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          {attachment.mime_type} • {getDocumentTypeLabel(attachment.document_type)}
                        </p>
                      </div>
                      <span className="rounded-full bg-white px-3 py-1 text-xs text-slate-600">
                        {getTextExtractStatusLabel(attachment.text_extract_status)}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-4 py-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        {getDocumentTypeLabel(selectedAttachment?.document_type) || 'Tệp PDF'}
                      </p>
                    </div>
                    <a
          ...
          <div className="space-y-6">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs leading-5 text-slate-500">
                          {getFactFieldLabel(fact.field_name)}
                        </p>
                        <p className="mt-1 font-medium text-slate-900">{fact.field_value}</p>
                      </div>
                      <span className="rounded-full bg-white px-3 py-1 text-xs text-slate-600">
                        {getDocumentTypeLabel(fact.document_type || fact.source_type)}
                      </span>
                    </div>
                    <div className="mt-2 text-sm text-slate-500">
                      <p>Nguồn trích xuất: {fact.source_label || fact.source_type}</p>
                      <p>Thời điểm ghi nhận: {formatDateTime(fact.source_sent_at)}</p>
                    </div>
                  </div>
                ))}
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 sm:p-8">
              <h2 className="text-xl font-semibold text-slate-900">Container liên kết</h2>
